...
const copy = copyVariants.variantB;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
            <a
              ...
              onClick={(e) => { e.preventDefault(); handleAnchorClick('benefits'); }}
            >
            ...
            <a
              href="#steps"
              className="transition hover:text-slate-900"
              onClick={(e) => { e.preventDefault(); handleAnchorClick('steps'); }}
            >
            ...
            <a
              href="#plans"
              className="transition hover:text-slate-900"
              onClick={(e) => { e.preventDefault(); handleAnchorClick('plans'); }}
            >
              {copy.header.nav.plans}
            </a>
            <a
              href="#faq"
              className="transition hover:text-slate-900"
      ...
      <main>
        ...
        >
          <div className="mx-auto grid max-w-6xl gap-12 sm:gap-16 px-4 sm:px-6 py-16 sm:py-20 lg:items-center lg:py-28 relative z-10 lg:grid-cols-2">
            <div className="space-y-8">
              <div className="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold uppercase tracking-wide rounded-full bg-sky-100 text-sky-700">
                {copy.hero.badge}
              </div>
              <div className="space-y-6">
                <h1 className="text-4xl font-semibold md:text-5xl text-slate-900">
                  {copy.hero.title}
                </h1>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="relative">
                          <svg className="h-20 w-20 -rotate-90 transform lg:h-24 lg:w-24">
                            <circle cx="40" cy="40" r="36" fill="none" stroke="currentColor" strokeWidth="6" className="text-slate-700/50" />
                            <circle cx="40" cy="40" r="36" fill="none" stroke="currentColor" strokeWidth="6" strokeDasharray={`${2 * Math.PI * 36}`} strokeDashoffset={`${2 * Math.PI * 36 * 0.7}`} className="text-sky-400" strokeLinecap="round" />
                          </svg>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        return (
                          <div key={idx} className="rounded-xl bg-slate-800/50 p-3 backdrop-blur-sm">
                            ...
                            <div className="h-2 overflow-hidden rounded-full bg-slate-700/50">
                              <div className="h-full rounded-full bg-gradient-to-r from-sky-400 to-blue-500 transition-all" style={{ width: `${percentage}%` }} />
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-7 gap-1.5">
                      {[
                        ...
                        >
                          <p className={`mb-1 text-[10px] font-semibold ${item.isToday ? 'text-orange-300' : 'text-slate-400'}`}>{item.day}</p>
                          <p className={`mb-1 text-xs font-semibold ${item.isToday ? 'text-white' : 'text-slate-300'}`}>{item.date}</p>
                          {item.type === 'workout' ? (
                            <div className="space-y-0.5">
                              <div className={`mx-auto h-1.5 w-1.5 rounded-full ${item.intensity === 'leve' ? 'bg-green-400' : item.intensity === 'mod' ? 'bg-yellow-400' : 'bg-orange-400'}`} />
          ...
          <div className="mx-auto max-w-6xl px-6 py-16 md:py-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-6 md:gap-8">
              {[
                ...
                <div key={filename} className="group flex justify-center rounded-xl border border-slate-200 bg-slate-100 p-2 shadow-sm transition hover:shadow-lg hover:border-sky-200">
                  <img src={`/depoimentos/${filename}`} alt="Depoimento de cliente" className="w-full h-auto max-w-full object-contain transition group-hover:opacity-95" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative mx-auto max-w-6xl px-6 py-24 md:py-28">
            <div className="text-center">
              <div className="inline-flex items-center gap-2 rounded-full bg-sky-100/80 px-4 py-2 backdrop-blur-sm">
                ...
                <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">{copy.resultsProof.badge}</p>
              </div>
                      ...
                      >
                        ...
                        <div className="relative flex items-start gap-6">
                          ...
                          <div className="flex-1 pt-1">
                            ...
                            <p className="text-base leading-relaxed text-slate-700 md:text-lg">{point}</p>
                          </div>
                        </div>
                      </div>
                    );
                ...
                {copy.experienceSection.problems.map((problem, idx) => (
                  <div key={idx} className="flex items-start gap-4">
                    ...
                    <div className="flex-1 pt-0.5">
                      <p className="text-lg font-semibold leading-relaxed text-white">{problem}</p>
                    </div>
                  </div>
                ))}
              </div>
                    ...
                    <div key={step.title} className="relative flex items-stretch gap-6">
                      <article
                        className={`relative flex aspect-square w-64 min-w-[16rem] max-w-[16rem] flex-col rounded-3xl border bg-white/90 p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:w-60 sm:min-w-[15rem] md:w-56 md:min-w-[14rem] ${
                          isLast ? 'border-amber-200/70 bg-gradient-to-br from-white to-amber-50 shadow-lg shadow-amber-100' : 'border-slate-100'
                        }`}
                        style={{ zIndex: 1 }}
                      >
                        <div className={`mb-6 inline-flex h-12 w-12 items-center justify-center rounded-full text-lg font-semibold shadow-sm ${isLast ? 'bg-amber-400/20 text-amber-600 shadow-amber-200' : 'bg-white text-slate-900 shadow-slate-200'}`}>
                          {index + 1}
                        </div>
                        <h3 className="text-lg font-semibold text-slate-900">{step.title}</h3>
                        <p className="mt-4 text-sm text-slate-600">{step.description}</p>
                        {isLast && <span className="absolute right-6 top-6 text-amber-400">✨</span>}
                      </article>
          ...
          <div className="relative mx-auto max-w-6xl px-6 py-24 md:py-28">
            <div className="text-center">
              <div className="inline-flex items-center gap-2 rounded-full bg-red-500/20 px-4 py-2 backdrop-blur-sm border border-red-400/30">
                ...
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-red-300">Compare os valores</p>
              </div>
            </div>
                  ...
                  <div className="mx-auto grid max-w-3xl gap-6 sm:grid-cols-2">
                    {copy.pricingAnchor.benefits.map((benefit, idx) => (
                      <div key={idx} className="group flex items-start gap-4 rounded-2xl bg-slate-800/50 p-5 backdrop-blur-sm border border-sky-500/20 transition-all hover:border-sky-400/40 hover:bg-slate-800/70">
                        ...
                        <div className="flex-1 pt-1">
                          <p className="text-lg font-semibold leading-relaxed text-white">{benefit}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {copy.plans.plans.map((plan) => (
                <article
                  ...
                  className={`flex h-full flex-col rounded-3xl border p-6 text-center shadow-[0_30px_80px_-50px_rgba(12,74,110,0.6)] transition hover:-translate-y-1 ${
                    plan.highlight ? 'border-sky-300 bg-white shadow-sky-200' : 'border-slate-200 bg-white'
                  }`}
                >
                  <p className="text-sm font-semibold uppercase tracking-widest text-sky-600">{plan.name}</p>
                  <h3 className="mt-4 text-2xl font-semibold text-slate-900">{plan.price}</h3>
                  ...
                  <a
                    ...
                    className={`mt-10 w-full rounded-full px-6 py-3 text-sm font-semibold transition text-center block ${
                      plan.highlight ? 'bg-sky-600 text-white hover:bg-sky-500' : 'border border-slate-200 text-slate-800 hover:border-sky-300 hover:text-sky-600'
                    }`}
                  >
                    {plan.cta}
                  </a>
          ...
          <div className="mx-auto max-w-4xl px-6 py-20">
            ...
            <div className="mt-12 space-y-6">
              {copy.faq.faqs.map((faq) => (
                <details key={faq.question} className="group rounded-2xl border border-slate-200 bg-slate-50 p-6 transition">
